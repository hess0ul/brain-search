...
jamais par la taille du vault.** Un seul script bash sans dépendance, recalculé à la volée (donc
toujours frais), donne à Claude cinq vues bornées :
...
- 🕒 **`recent`** — notes modifiées ces N jours, plus récentes d'abord (se remettre à jour / post-`/compact`).
- 📦 **`gather`** — concatène le corps des N meilleures notes en un bloc prêt-à-raisonner.
...
```text
# 🕒 Récent — notes modifiées (≤ 14 j), plus récentes d'abord
...
Prend les 5 notes les plus pertinentes (même classement que `find`) et concatène leur corps (frontmatter
retiré, 60 lignes/note max) en un bloc — « la demande + les notes pertinentes », chargé d'un coup au lieu
d'ouvrir cinq notes une par une.
