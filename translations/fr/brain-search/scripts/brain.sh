...
#   brain.sh recent [N]          notes modifiées ces N jours (défaut 14), plus récentes d'abord
#   brain.sh gather <terme>      AGRÈGE le corps des 5 notes les + pertinentes → bloc prêt-à-raisonner
...
# Dépendances : bash, awk, find, sort, perl (standard sur macOS/Linux/WSL/Git-Bash).
set -uo pipefail
...
rank_records() { # $1=requête en minuscules -> "score\037rel\037title\037type\037status\037tags\037snip" trié
  local q="$1"
...
cmd_recent() {
  ...
  printf '# 🕒 Récent — notes modifiées (≤ %s j), plus récentes d'\''abord\n\n' "$days"
  while IFS=$'\t' read -r date rel; do
    [ "$n" -ge 40 ] && { printf '\n… (40 plus récentes affichées)\n'; break; }
  ...
  [ "$n" -eq 0 ] && printf '(aucune note modifiée sur la période)\n'
  printf '\n— %d note(s) sur ≤ %s j\n' "$n" "$days"
}

cmd_gather() {
  [ -n "$QUERY" ] || { echo "usage: brain.sh gather <terme>" >&2; return 1; }
  local recs; recs=$(rank_records "$(printf '%s' "$QUERY" | tr 'A-Z' 'a-z')")
  [ -z "$recs" ] && { echo "(aucune note pertinente à agréger)"; return; }
  printf '# 📦 Agrégat « %s » — corps des 5 notes les plus pertinentes\n' "$QUERY"
  printf '%s\n' "$recs" | head -5 | awk -F'\037' '{print $2}' | while IFS= read -r rel; do
    [ -f "$rel" ] || continue
    printf '\n---\n## %s\n\n' "$rel"
    awk 'NR==1&&$0=="---"{fm=1;next} fm==1&&$0=="---"{fm=2;next} fm==1{next} {print}' "$rel" | sed '/^$/N;/^\n$/D' | head -60
  done
  printf '\n---\n(corps tronqué à 60 lignes/note ; ouvre la note pour le détail complet)\n'
}
...
case "$CMD" in
  ...
  gather) cmd_gather ;;
  audit)  cmd_audit ;;
  *) echo "commandes: map | find <terme> | recent [N] | gather <terme> | audit" >&2; exit 2 ;;
esac
